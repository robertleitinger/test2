...
- **Transiente FTP-Timeouts abfangen:** Hostinger drosselt wiederholte FTP-Verbindungen → Fehler „Timeout (control socket)". Lösung: Upload mit **automatischen Wiederholversuchen + Backoff** (Bash-Loop um `lftp`, oder `nick-fields/retry`). Für Accounts ohne FTPS: Plain FTP erzwingen (`set ftp:ssl-allow no`).
